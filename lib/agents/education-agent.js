...
export class EducationAgent extends BaseAgent {
  ...
  calculateEducationScore(candidateTier, requiredTier, fieldRelevance) {
    // If no education required, score depends on field relevance
    if (requiredTier === 0) {
      // No requirement specified - check if field is relevant
      if (fieldRelevance.relevant) {
        // Relevant field (e.g., CS degree for tech job): 75%
        return 75;
      } else {
        // Unrelated field (e.g., healthcare degree for tech job): 40%
        // Not penalizing too harshly, but making it clear it's not ideal
        return 40;
      }
    }
    ...
    if (candidateTier === requiredTier) {
      // Base 100% for exact match
      if (fieldRelevance.relevant) {
        return 100; // Perfect: right level AND relevant field
      } else {
        return 75; // Right level but unrelated field
      }
    }

    // Exceeds requirement
    if (candidateTier > requiredTier) {
      // Overqualification is acceptable
      if (fieldRelevance.relevant) {
        return 100; // Excellent: over-qualified AND relevant
      } else {
        return 75; // Over-qualified but unrelated field
      }
    }
    ...
    // Additional penalty if field is unrelated
    if (!fieldRelevance.relevant) {
      score = Math.max(0, score - 15); // Extra penalty for unrelated field
    }
